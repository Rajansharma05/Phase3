...
public class VaccinationCenterDtoValidator implements ConstraintValidator<VaccinationCenterValidator, VaccinationCenterDto> {
    ...
    public static RequestDtoValidator<VaccinationCenterDto> getRequestDtoValidator(VaccinationCenterDto dto) {
        ...
        if (dto.getHttpMethod().equalsIgnoreCase("post")) return new PostRequestValidator();
        if (dto.getHttpMethod().equalsIgnoreCase("put")) return new PutRequestValidator();
        return null;
    }
    ...
    private static class PostRequestValidator implements RequestDtoValidator<VaccinationCenterDto> {
        public boolean validate(VaccinationCenterDto center, ConstraintValidatorContext context) {
            ...
            Predicate<VaccinationCenterDto> validateNameLength = center1 ->
                    center1.getName().length() > 4 && center1.getName().length() < 60;

            if (validateNameLength.negate().test(center)) {
                context.disableDefaultConstraintViolation();
                context.buildConstraintViolationWithTemplate("Center Name Should be between 4 and 60 characters")
            ...
            Predicate<VaccinationCenterDto> validateAddressLength = center1 ->
                    center1.getCity().length() > 3 && center1.getCity().length() < 100;

            if (validateAddressLength.negate().test(center)) {
                context.disableDefaultConstraintViolation();
                context.buildConstraintViolationWithTemplate("Center City Should be between 15 and 100 characters")
