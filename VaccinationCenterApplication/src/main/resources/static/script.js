...
// Define an object that holds functions for managing citizen-related actions
let citizenApi = {
    list: citizenList,
...
// Perform a logout action and redirect to the login page
function logout() {
    localStorage.removeItem("token");
    window.location.href = getBaseUrl() + "/login";
}

// Redirect to the login page if response status is 302
...
// Construct and return headers for API requests
function getHeaders() {
...
function citizenListByUrl(url, deleteAction, tableId) { 
	let tableRow = { citizensTable: function (citizen) { 
		return `<td>${citizen.id}</td> <td>${citizen.name}</td> 
		<td>${citizen.city}</td> <td>${citizen.doesCount}</td> 
		<td>${citizen.vaccinationStatus}</td> <td>${citizen.center?.name}
		</td>
		<td> 
		...
		<button id="${citizen.id}" onclick="${deleteAction.name}(event)">Delete</button> 
			...
			return `<td>${citizen.id}</td> <td>${citizen.name}</td> 
			<td> <button id="${citizen.id}" onclick="_citizenView(event)">View</button> 
			</td>`; 
			}, 
		};
			...
			fetch(getBaseUrl() + "/vaccinationcenter/" + centerId, { 
				headers: getHeaders(), }) .then((response) => { 
					...
					if (response.ok) { 
						...
						}) .then((center) => { let centerDetails = ` <ul class="list-group"> 
...
getHeaders(), }) .then(function (response) { redirectToLogin(response); 
if (response.ok) { return response.json(); } 
else { throw new Error("Error retrieving citizen data."); } }) .then(function (citizens) { 
	// Populate the table with citizen data 
let message = ""; if (citizens.length > 0) { 
	message = `<div class="alert alert-warning" role="alert"> Total ${citizens.length} citizens found. </div>`; 
let countRowElement = document.getElegetElementById("countRow");
countRowElement.innerHTML = message; } 
var tableBody = document .getElementById(listTableId) .querySelector("tbody"); 
citizens.forEach(function (citizen) { var row = document.createElement("tr"); 
row.innerHTML = tableRow[listTableId](citizen); tableBody.appendChild(row); }); }) 
...
.then(function (response) { 
	...
	if (response.ok) { 
		return response.json(); } 
		else { throw new Error("Error deleting citizen data."); } }) 
		.then(function (data) { 
...
function deleteCitizen(id, callback) {
	 let baseUrl = document.getElementById("baseUrl");
	  ...
	  fetch(baseUrl.value + "/citizens/" + id, {
		  ...
		  .then(function (response) { redirectToLogin(response); 
		  if (response.ok) { return response.json(); } 
		  else { throw new Error("Error deleting citizen data."); } }) 
...
function _layoutCitizenOnNew() {
	...
	fetch(baseUrl.value + "/vaccinationcenter", {
	 ...
	 .then(function (centers) { console.log(centers); 
	 console.log("design form"); _layoutAppendCenters(centers); }) 
...
function _layoutAppendCenters(centers, citizen) { 
	...
	let centerOptions = centers; centerOptions.forEach(function (option) { 
		var optionElement = document.createElement("option"); 
		optionElement.value = option.id; 
		optionElement.text = option.id + " | " + option.name + " | " + option.address; 
		center.appendChild(optionElement); }); 
		// Select a default option 
		let defaultOption = citizen.center?.id; 
		// Set the value of the default option here 
		if (defaultOption) { center.value = defaultOption; } } 
...
function _layoutOneCitizenView(citizen) { 
				...
				let listItems = ` <ul id="oneCitizenView" class="list-group"> 
				<li class="list-group-item"> <label htmlFor="id">ID: 
				</label> <span>${citizen.id}</span> </li> 
				<li class="list-group-item"> 
				<label htmlFor="name">Name: </label> <span>${citizen.name}</span> </li> 
				<li class="list-group-item"> <label htmlFor="city">City: </label> <span>${citizen.city}</span> </li> 
				<li class="list-group-item">
<label htmlFor="count">Vaccination Count: </label> 
<span>${citizen.doesCount}</span> </li> 
<li class="list-group-item"> <label htmlFor="status">Status: </label> 
<span>${citizen.vaccinationStatus}</span> </li> 
<li class="list-group-item"> <label htmlFor="centerName">Vaccination Center: </label> 
<span>${citizen.center?.name}</span> </li> 
<li class="list-group-item"> <label htmlFor="centerAddress">Vaccination Center Address: </label> 
<span>${citizen.center?.address}</span> </li>
</ul> `; citizenElement.innerHTML = listItems; }
...
function centerFormAdd(event) { 
	...
	errorListElement.innerHTML = ""; errorListElement.className = ""; 
	// Get form data 
	...
	const payload = { name: centerName, city: centerCity, httpMethod: "POST", }; 
	let methodName = "POST"; let centerId = document.getElementById("centerId").value; 
	if (centerId) { methodName = "PUT"; 
	...
	fetch(getBaseUrl() + "/vaccinationcenter", { 
		...
		.then((response) => { 
			...
			if (response.ok) { 
				...
				return response.json(); } else if (response.status === 400) { 
					// Bad Request, handle errors 
					return response.json().then((data) => { 
						if (Array.isArray(data.errors)) { 
							// Display error messages as warnings 
							let errorLabels = ""; data.errors.forEach((error) => {
errorLabels += `<div class="alert alert-warning" role="alert">${error.message}</div>`; });
 ...
 else { console.warn("Warning: Bad Request"); } 
...
function vaccinationCenters() {
	...
	// Fetch citizen data from API 
	let baseUrl = document.getElementById("baseUrl"); 
	console.log(baseUrl.value); fetch(getBaseUrl() + "/vaccinationcenter", {
		 method: "GET", headers: getHeaders(), }) .then(function (response) { 
			 redirectToLogin(response); 
			 if (response.ok) { 
				 return response.json(); 
				 } else 
...
.then(function (centers) { 
	...
	let message = ""; 
	if (centers.length > 0) { 
		message = `<div class="alert alert-warning" role="alert"> Total ${centers.length} 
		vaccination center found. </div>`; 
		let countRowElement = document.getElementById("countRow"); 
		countRowElement.innerHTML = message; } 
		var tableBody = document .getElementById("centerTable") .querySelector("tbody"); 
		centers.forEach(function (center) {
			 var row = document.createElement("tr"); 
			 row.innerHTML = ` <td>${center.id}</td> 
			 <td>${center.name}</td> 
			 <td>${center.address}</td> 
			 <td> <button id="${center.id}" onclick="_centerView(event)">View</button> 
			 <button id="${center.id}" onclick="_centerEdit(event)">Edit</button> 
			 <button id="${center.id}" onclick="_centerDelete(event)">Delete</button> </td> `; 
...
fetch(getBaseUrl() + "/vaccinationcenter/" + id, { 
	headers: getHeaders(), }) .then((response) => { 
		...
		if (response.ok) { 
			...
			.then((data) => { bindDataWithEditForm(data); }) 
			.catch((error) => { console.log(error); }); } 
...
let city = center.address; let selectedCityValue; 
...
var value = option.value; var text = option.text; 
if (city === text) { selectedCityValue = value; break; } 
// Do something with the option 

console.log("Option value: " + value); 
console.log("Option text: " + text); } 
if (selectedCityValue) { select.value = selectedCityValue; } 
...
fetch(getBaseUrl() + "/vaccinationcenter/" + id, {
	 ...
	 if (response.ok) { return response.json(); } 
	 else { throw new Error("Error while deleting vaccinationcenter: " + id); } }) 
	 .then((data) => { vaccinationCenters(); }) 
	 .catch((error) => { console.log(error); }); } 

function saveCitizen(event) { event.preventDefault(); 
...
let methodName = "POST"; 
const payload = { name: citizenName, city: citizenCity,
doesCount: 0, httpMethod: methodName, centerId: centerId, doesCount: vaccinationCountSelectValue, };
 if (citizenId) { methodName = "PUT"; payload.httpMethod = "PUT"; payload.id = citizenId; }
 
// Make a POST request to the vaccinationcenter endpoint 

fetch(getBaseUrl() + "/citizens", { 
	method: methodName, headers: getHeaders(), body: JSON.stringify(payload), }) 
	.then((response) => { redirectToLogin(response); if (response.ok) { 
 ...
 errorListElement.innerHTML = errorLabels; 
 // document.getElementById("centerForm").appendChild(errorListElement); 
 } else { console.warn("Warning: Bad Request"); } throw new Error("Bad Request"); }); } else { 
	 // Handle other response status codes
throw new Error(`Request failed with status ${response.status}`); } }) 
.then((data) => { console.log("Response:", data); 
let messageElement = document.getElementById("errorList"); 
messageElement.innerHTML = ` <div class="alert alert-success" role="alert"> Citizen Saved </div> `; 
setTimeout(() => { let messageElement = document.getElementById("errorList"); messageElement.innerHTML = ""; }, 2000);
...
// Handle the response as needed
 ...
 function _newCenter(event) { display("centers", "none"); 
 display("centerForm", "block"); } 
...
fetch("/registration", { method: "POST", body: JSON.stringify(payload), headers: 
{ "Content-Type": "application/json", Accept: "application/json", }, }) 
.then((response) => { if (response.ok) { 
	onActionMessage( `<div class="alert alert-success" role="alert">Registration Success.</div>` ); 
	return response.json(); } console.log(response); if (response.status == 400) { 
		// Bad Request, handle errors 
		return response.json().then((data) => { 
			if (Array.isArray(data.errors)) { 
				let errorLabels = ""; data.errors.forEach((error) => { 
...
else { onActionMessage( `<div class="alert alert-warning" role="alert">Registration Failed. Try again!!</div>` ); } }) 
...
.catch((error) => { 
	...
	function onActionMessage(content) { 
		...
		setTimeout(() => { messageListElement.innerHTML = ""; }, 2000); }